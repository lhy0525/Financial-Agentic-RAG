...
Usage:
    ...
    # Bulk index local prospectus PDFs for the local financial chat demo
    python scripts/ingest.py --path D:/pdfs/prospectus --financial-prospectus
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--collection", "-c",
        default=None,
        help="Collection name for organizing documents (default: 'default', or financial_platform.prospectus_collection with --financial-prospectus)"
    )

    parser.add_argument(
        "--financial-prospectus",
        action="store_true",
        help="Bulk ingest local prospectus PDFs into financial_platform.prospectus_collection (default: prospectus_uploads)."
    )
...
def print_summary(results: List[PipelineResult], verbose: bool = False) -> None:
    ...
    skipped = sum(1 for r in results if r.success and r.stages.get("integrity", {}).get("skipped", False))
...
def resolve_collection(args: argparse.Namespace, config_path: Path) -> str:
    if args.collection:
        return args.collection
    if not args.financial_prospectus:
        return "default"
    ...
    try:
        ...
        data = {}
    platform = data.get("financial_platform", {})
    if not isinstance(platform, dict):
        platform = {}
    return str(platform.get("prospectus_collection") or "prospectus_uploads")
...
def main() -> int:
    ...
    try:
        ...
        collection = resolve_collection(args, config_path)
    ...
    if args.financial_prospectus:
        print("   Local origin: bulk_local_directory")
    ...
    try:
        pipeline = IngestionPipeline(
            ...
            force=args.force,
            extra_metadata={"local_origin": "bulk_local_directory"} if args.financial_prospectus else None,
        )
    except Exception as e:
