...
class IngestionPipeline:
    ...
    def __init__(
        ...
        # Stage 4: Transforms
        self.chunk_refiner = ChunkRefiner(settings) if run_transforms else _NoOpTransform()
        logger.info(f"  ✓ ChunkRefiner initialized (use_llm={self.chunk_refiner.use_llm})")
        
        self.metadata_enricher = MetadataEnricher(settings) if run_transforms else _NoOpTransform()
        logger.info(f"  ✓ MetadataEnricher initialized (use_llm={self.metadata_enricher.use_llm})")
        
        self.image_captioner = ImageCaptioner(settings) if run_transforms else _NoOpTransform()
        ...
        configured_batch_size = settings.ingestion.batch_size if settings.ingestion else 100
        batch_size = embedding_batch_size or configured_batch_size
    ...
    def run(
        ...
        try:
            ...
            if not self.force and self.integrity_checker.should_skip(file_hash, collection=self.collection):
            ...
            self.integrity_checker.mark_failed(file_hash, str(file_path), str(e), collection=self.collection)
