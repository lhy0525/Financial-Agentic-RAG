...
class FileIntegrityChecker(ABC):
    ...
    def should_skip(self, file_hash: str, collection: Optional[str] = None) -> bool:
        ...
        Args:
            ...
            collection: Optional collection/namespace identifier. When
                provided, only a success record in that collection is skipped.
...
class SQLiteIntegrityChecker(FileIntegrityChecker):
    ...
    def _ensure_database(self) -> None:
        ...
        try:
            ...
            """)
            self._migrate_collection_primary_key(conn)
    ...
    def _migrate_collection_primary_key(self, conn: sqlite3.Connection) -> None:
        """Migrate older hash-only ingestion history to collection-aware keys."""
        cursor = conn.execute("PRAGMA table_info(ingestion_history)")
        columns = cursor.fetchall()
        column_names = {row[1] for row in columns}
        pk_columns = [row[1] for row in columns if row[5] > 0]
        if pk_columns == ["file_hash", "collection"]:
            return

        conn.execute("""
            CREATE TABLE IF NOT EXISTS ingestion_history_new (
                file_hash TEXT NOT NULL,
                file_path TEXT NOT NULL,
                status TEXT NOT NULL,
                collection TEXT,
        ...
        """)
        collection_expr = "COALESCE(collection, 'default')" if "collection" in column_names else "'default'"
        conn.execute(f"""
            INSERT OR REPLACE INTO ingestion_history_new
            (file_hash, file_path, status, collection, error_msg, processed_at, updated_at)
            SELECT file_hash, file_path, status, {collection_expr},
                   error_msg, processed_at, updated_at
            FROM ingestion_history
        """)
        conn.execute("DROP TABLE ingestion_history")
        conn.execute("ALTER TABLE ingestion_history_new RENAME TO ingestion_history")
    ...
    def should_skip(self, file_hash: str, collection: Optional[str] = None) -> bool:
        ...
        try:
            if collection is None:
                ...
                cursor = conn.execute(
                    "SELECT 1 FROM ingestion_history WHERE file_hash = ? AND collection = ? AND status = 'success'",
    ...
    def mark_success(
        ...
        try:
            ...
            normalized_collection = collection or "default"
            cursor = conn.execute(
                "SELECT processed_at FROM ingestion_history WHERE file_hash = ? AND collection = ?",
                (file_hash, normalized_collection),
            )
            result = cursor.fetchone()
            ...
            if result:
                ...
                """, (file_path, normalized_collection, now, file_hash, normalized_collection))
            else:
                # Insert new record
                conn.execute("""
            ...
            if result:
                ...
                conn.execute("""
                    ...
                    SET file_path = ?,
                        ...
                        updated_at = ?
                    WHERE file_hash = ? AND collection = ?
                """, (file_path, error_msg, now, file_hash, normalized_collection))
            else:
                # Insert new record
                conn.execute("""
                    INSERT INTO ingestion_history
