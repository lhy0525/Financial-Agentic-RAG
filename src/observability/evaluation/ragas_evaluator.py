...
OPENAI_COMPATIBLE_PROVIDERS = {"openai", "zhipu"}
...
class RagasEvaluator(BaseEvaluator):
    ...
    def _build_wrappers(self) -> tuple:
        ...
        if use_azure_llm:
            ...
            resolved_base_url = llm_base_url or (
                DEFAULT_ZHIPU_BASE_URL if provider == "zhipu" else None
            )
            llm_client = AsyncOpenAI(api_key=llm_cfg.api_key, base_url=resolved_base_url)
        else:
            raise ValueError(
        ...
        elif emb_provider in OPENAI_COMPATIBLE_PROVIDERS:
            resolved_base_url = emb_base_url or (
                DEFAULT_ZHIPU_BASE_URL if emb_provider == "zhipu" else None
            )
            emb_client = AsyncOpenAI(api_key=emb_cfg.api_key, base_url=resolved_base_url)
        else:
            raise ValueError(
