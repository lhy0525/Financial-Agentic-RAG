...
def test_process_can_fail_fast_on_batch_failure():
    """Test fail-fast mode stops before returning partially aligned vectors."""
    dense = FakeDenseEncoder()
    sparse = FakeSparseEncoder()
    ...
    chunks = [Chunk(id=f"{i}", text=f"text {i}", metadata={"source_path": "test.pdf"}) for i in range(6)]
    trace = TraceContext()

    with pytest.raises(RuntimeError, match="Batch 1 failed.*Simulated batch failure"):
        processor.process(chunks, trace=trace)

    assert trace.get_stage_data("batch_1_error")["batch_size"] == 2
