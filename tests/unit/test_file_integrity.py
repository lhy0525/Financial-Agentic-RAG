...
class TestSQLiteIntegrityChecker:
    ...
    def test_should_skip_is_collection_aware(self, checker, temp_file):
        """Test same file hash is skipped only in collections where it succeeded."""
        file_hash = checker.compute_sha256(temp_file)

        checker.mark_success(file_hash, temp_file, collection="default")

        assert checker.should_skip(file_hash, collection="default") is True
        assert checker.should_skip(file_hash, collection="prospectus_uploads") is False

        checker.mark_success(file_hash, temp_file, collection="prospectus_uploads")

        assert checker.should_skip(file_hash, collection="prospectus_uploads") is True

        conn = sqlite3.connect(checker.db_path)
        try:
            cursor = conn.execute(
                "SELECT collection FROM ingestion_history WHERE file_hash = ? ORDER BY collection",
                (file_hash,),
            )
            assert [row[0] for row in cursor.fetchall()] == ["default", "prospectus_uploads"]
        finally:
            conn.close()
    ...
    def test_migrates_legacy_hash_only_schema_without_collection_column(self, temp_db):
        """Test migration from pre-collection ingestion history schema."""
        ...
        try:
            ...
            conn.execute(
                ...
                (file_hash, file_path, status, error_msg, processed_at, updated_at)
                VALUES (?, ?, 'success', NULL, ?, ?)
                """,
                (file_hash, "legacy.pdf", now, now),
            )
            conn.commit()
        finally:
            conn.close()
